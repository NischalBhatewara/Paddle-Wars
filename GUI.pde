...
class MultipleButtons extends Widget
{
  public Toggle [] buttons;  

  MultipleButtons (int numButtons, int x, int y, int w, int h, int orientation)
  {
    super("", x, y, w*numButtons, h);
    buttons = new Toggle[numButtons];    
    for (int i = 0; i < buttons.length; i++)
    ...
    bgColor = inactiveColor;
    for (int i = 0; i< buttons.length;i++) {
      buttons[i].setInactiveColor(bgColor);
      buttons[i].setActiveColor(bgColor);
    }
  }

  void setOneName(String name, int i) {
